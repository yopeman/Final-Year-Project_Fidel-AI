...
from app.model.reading_skill import ReadingSkill
...
def _load_seeded_reading_skills(db):
    return (
        db.query(ReadingSkill)
        .join(Skill, ReadingSkill.skill_id == Skill.id)
...
def seed_reading_skills(db, skill_ids=None):
    """Seed 25 reading skill assessments (one per seeded skill)."""
    existing = _load_seeded_reading_skills(db)
    if len(existing) >= SEED_COUNT:
        return [rs.id for rs in existing]
    ...
    if reading_to_add:
        db.add_all(reading_to_add)
        db.flush()

    return [rs.id for rs in _load_seeded_reading_skills(db)]
