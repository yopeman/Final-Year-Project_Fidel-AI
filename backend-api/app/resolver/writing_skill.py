...
def resolve_coherence(writing_skill_obj, info):
    return writing_skill_obj.coherence.value if writing_skill_obj.coherence else None

@writing_skill.field("grammar")
def resolve_grammar(writing_skill_obj, info):
    return writing_skill_obj.grammar.value if writing_skill_obj.grammar else None

@writing_skill.field("vocabulary")
def resolve_vocabulary(writing_skill_obj, info):
    return writing_skill_obj.vocabulary.value if writing_skill_obj.vocabulary else None

@writing_skill.field("punctuation")
def resolve_punctuation(writing_skill_obj, info):
    return writing_skill_obj.punctuation.value if writing_skill_obj.punctuation else None

@writing_skill.field("finalResult")
def resolve_final_result(writing_skill_obj, info):
    return writing_skill_obj.final_result.value if writing_skill_obj.final_result else None
